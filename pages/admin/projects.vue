      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">projects</th>
          ...
          <tbody class="bg-white divide-y divide-gray-100">
            <tr v-for="projects in projects" :key="projects.id" class="hover:bg-gray-50 transition-colors">
              <td class="px-6 py-5">
...
/>
                  ...
                  <div class="ml-4">
                    <div class="text-base font-semibold text-gray-900">{{ projects.title }}</div>
                    <div class="text-base font-semibold text-gray-900">{{ projects.links }}</div>
                    <div class="text-sm text-gray-500 line-clamp-2 mt-1">{{ projects.description }}</div>
                  </div>
                </div>
              </td>
       <td class="px-6 py-5">
  <div class="flex items-center">
    ...
    ></div>
    <span class="text-sm text-gray-700">{{ projects.color || 'No color' }}</span>
  </div>
</td>
              ...
              <td class="px-6 py-5 text-right">
                <div class="flex justify-end space-x-3">
                  ...
                  <button
                    @click="confirmDelete(projects.id)"
          ...
          <div class="bg-white px-6 py-5 sm:p-8">
            <div class="flex justify-between items-start">
              <h3 class="text-2xl font-bold text-gray-900 mb-6">
                {{ isEditing ? 'Edit projects' : 'Create New projects' }}
              </h3>
              <button 
                @click="closeModal" 
            ...
            <form class="space-y-6" @submit.prevent="saveprojects">
              <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div class="sm:col-span-2">
                  <label for="title" class="block text-sm font-medium text-gray-700 mb-1">projects Title *</label>
                  <input
                  ...
                  />
                                ...
                                <div class="sm:col-span-2">
                  <label for="title" class="block text-sm font-medium text-gray-700 mb-1">projects Links *</label>
                  <input
                    v-model="modalForm.links"
                    type="text"
                    id="links"
                    class="form-control block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-3 px-4 border"
                    placeholder="Enter projects title"
                ...
                <div class="sm:col-span-2">
                  ...
                  <input
                    ...
                    id="color"
                    class="form-control block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-3 px-4 border"
                    placeholder="Where is the projects happening?"
                    required
                  />
                </div>
                ...
                <div class="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    rows="4"
                    class="form-control block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-3 px-4 border"
                    placeholder="Tell people about your projects..."
                    required
                  ></textarea>
                </div>

                <div class="sm:col-span-2">
                  <label class="block text-sm font-medium text-gray-700 mb-2">projects Image</label>
...
/>
              ...
              <div class="flex justify-end space-x-3 pt-4">
                ...
                >
                  ...
                  {{ isEditing ? 'Update projects' : 'Create projects' }}
                </button>
              </div>
            </form>
          </div>
          ...
          <div class="bg-white px-6 py-8 sm:p-8">
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-xl font-bold text-gray-900">Delete projects</h3>
                <div class="mt-4">
                  <p class="text-gray-600">Are you sure you want to delete "<span class="font-medium">{{ projects.find(e => e.id === projectsToDelete)?.title || 'this projects' }}</span>"? This action cannot be undone.</p>
                </div>
              </div>
            </div>
          </div>
...
const projectsToDelete = ref(null);
...
const saveprojects = async () => {
  ...
  try {
    ...
    formData.append('links', modalForm.value.links);
    ...
    let url = isEditing.value 
      ? `${apiBaseUrl}/projects/${modalForm.value.id}`
      : `${apiBaseUrl}/projects`;
    ...
    const savedprojects = response.data.projects || response.data;
    
    if (isEditing.value) {
      const index = projects.value.findIndex(e => e.id === modalForm.value.id);
      if (index !== -1) {
        projects.value[index] = savedprojects;
      }
            toast.add({
    title: 'success',
    description: 'projects uploaded successfully',
    color: 'success'
  })
        closeModal(); // This should close the modal
    } else {
      projects.value.unshift(savedprojects);
            toast.add({
    title: 'success',
...
const deleteprojects = async () => {
  ...
  try {
    await axios.delete(`${apiBaseUrl}/projects/${projectsToDelete.value}`, {
      headers: {
        Authorization: `Bearer ${authStore.token}`
      }
    });
    
    projects.value = projects.value.filter(e => e.id !== projectsToDelete.value);
              toast.add({
    title: 'success',
