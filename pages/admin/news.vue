      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">news</th>
          ...
          <tbody class="bg-white divide-y divide-gray-100">
            <tr v-for="news in news" :key="news.id" class="hover:bg-gray-50 transition-colors">
              <td class="px-6 py-5">
...
/>
                  ...
                  <div class="ml-4">
                    <div class="text-base font-semibold text-gray-900">{{ news.title }}</div>
                    <div class="text-sm text-gray-500 line-clamp-2 mt-1">{{ news.description }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-5">
                <div class="flex items-center">
                  <span class="text-sm text-gray-700">{{ news.date || '' }}</span>
                </div>
              </td>
              ...
              <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                  ...
                  <input
                    ...
                    id="date"
                    class="form-control block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-3 px-4 border"
                    placeholder="Where is the news happening?"
                    required
                  />
                </div>
                ...
                <div class="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    rows="4"
                    class="form-control block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-3 px-4 border"
                    placeholder="Tell people about your news..."
                    required
                  ></textarea>
                </div>
          ...
          <div class="bg-white px-6 py-8 sm:p-8">
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-xl font-bold text-gray-900">Delete news</h3>
                <div class="mt-4">
                  <p class="text-gray-600">Are you sure you want to delete "<span class="font-medium">{{ news.find(e => e.id === newsToDelete)?.title || 'this news' }}</span>"? This action cannot be undone.</p>
                </div>
              </div>
            </div>
          </div>
...
const savenews = async () => {
  ...
  try {
    ...
    let url = isEditing.value 
      ? `${apiBaseUrl}/news/${modalForm.value.id}`
      : `${apiBaseUrl}/news`;
    ...
    const savednews = response.data.news || response.data;
    
    if (isEditing.value) {
      const index = news.value.findIndex(e => e.id === modalForm.value.id);
      if (index !== -1) {
...
const deletenews = async () => {
  ...
  try {
    await axios.delete(`${apiBaseUrl}/news/${newsToDelete.value}`, {
      headers: {
        Authorization: `Bearer ${authStore.token}`
      }
    });
    
    news.value = news.value.filter(e => e.id !== newsToDelete.value);
              toast.add({
    title: 'success',
