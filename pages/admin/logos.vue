<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
      <div>
        <h1 class="text-3xl font-bold text-gray-800">logos Management</h1>
      </div>
      <button
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">logos</th>
          ...
          <tbody class="bg-white divide-y divide-gray-100">
            <tr v-for="logos in logos" :key="logos.id" class="hover:bg-gray-50 transition-colors">
              <td class="px-6 py-5">
...
/>
            ...
            <tr v-if="logos.length === 0 && !loading">
              <td colspan="3" class="px-6 py-8 text-center">
                <div class="text-gray-500 flex flex-col items-center">
                  <i class="fas fa-calendar-times text-4xl text-gray-300 mb-3"></i>
                  <p class="text-lg">No logos found</p>
                  <button 
                    @click="openCreateModal"
                    class="mt-4 text-blue-600 hover:text-blue-800 font-medium"
                  >
                    Create your first logos
                  </button>
                </div>
              </td>
            </tr>
          ...
          <div class="bg-white px-6 py-5 sm:p-8">
            <div class="flex justify-between items-start">
              <h3 class="text-2xl font-bold text-gray-900 mb-6">
                {{ isEditing ? 'Edit logos' : 'Create New logos' }}
              </h3>
              <button 
                @click="closeModal" 
            ...
            <form class="space-y-6" @submit.prevent="savelogos">
              <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
       

                <div class="sm:col-span-2">
                  <label class="block text-sm font-medium text-gray-700 mb-2">logos Image</label>
...
/>
              ...
              <div class="flex justify-end space-x-3 pt-4">
                ...
                >
                  ...
                  {{ isEditing ? 'Update logos' : 'Create logos' }}
                </button>
              </div>
            </form>
          </div>
          ...
          <div class="bg-white px-6 py-8 sm:p-8">
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-xl font-bold text-gray-900">Delete logos</h3>
                <div class="mt-4">
                  <p class="text-gray-600">Are you sure you want to delete "<span class="font-medium">{{ logos.find(e => e.id === logosToDelete)?.title || 'this logos' }}</span>"? This action cannot be undone.</p>
                </div>
              </div>
            </div>
          </div>
...
const fetchlogos = async () => {
  ...
  try {
    ...
    console.error('Error fetching logos:', error);
          toast.add({
    title: 'error',
    description: 'Failed to fetch logos data',
    color: 'danger'
  })
  } finally {
    loading.value = false;
...
const savelogos = async () => {
  ...
  try {
    ...
    let url = isEditing.value 
      ? `${apiBaseUrl}/logos/${modalForm.value.id}`
      : `${apiBaseUrl}/logos`;
    ...
    const savedlogos = response.data.data || response.data;
    
    if (isEditing.value) {
      const index = logos.value.findIndex(e => e.id === modalForm.value.id);
      if (index !== -1) {
        logos.value[index] = savedlogos;
      }
            toast.add({
    title: 'success',
    ...
    color: 'success'
      ...
      logos.value.unshift(savedlogos);
            toast.add({
    title: 'success',
    description: 'logos created successfully',
    color: 'success'
  })
        closeModal(); // This should close the modal
    }
    ...
    console.error('Error response:', error.response);
              toast.add({
    title: 'error',
    description: 'Failed to save logos',
    color: 'danger'
  })
  } finally {
...
const deletelogos = async () => {
  ...
  try {
    await axios.delete(`${apiBaseUrl}/logos/${logosToDelete.value}`, {
      headers: {
        Authorization: `Bearer ${authStore.token}`
      }
    });
    
    logos.value = logos.value.filter(e => e.id !== logosToDelete.value);
              toast.add({
    title: 'success',
    description: 'logos deleted successfully',
    color: 'success'
  })
  } catch (error) {
    console.error('Error deleting logos:', error);
          toast.add({
    title: 'error',
    description: 'Failed to delete logos',
    color: 'danger'
  })
  } finally {
    deleteLoading.value = false;
    showDeleteModal.value = false;
    logosToDelete.value = null;
  }
};
